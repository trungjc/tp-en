...
(function ($, window, document, undefined) {
  ...
  var defaults = {
    ...
    // seperate with a " "(space) to set an placeholder for each input box
    inputs: 4,
    ...
    inputmode: 'numeric',
    inputclass: '',
    // could be changed to form-control-lg or any other class
    characterwidth: null,
    // em width of PIN character; defaults to 0.54 (em width of a typical digit), or 0.4 if digits are hidden
    keydown: function keydown(e) {},
    change: function change(input, value, inputnumber) {// callback on every input on change (keyup event)
      //input = the input textbox DOM element
      //value = the value entered by user (or removed)
      //inputnumber = the position of the input box (in touch mode we only have 1 big input box, so always 1 is returned here)
    },
    complete: function complete(value, e, errorElement) {// callback when all inputs are filled in (keyup event)
  ...
  $.extend(Plugin.prototype, {
    ...
    buildInputBoxes: function buildInputBoxes() {
      ...
      if (this._isTouchDevice()) {
        // set main class
        $(this._container).addClass('touch'); // For touch devices we build a html table directly under the pincode textbox. The textbox will become transparent
        ...
        var input = $('<input>').attr({
          ...
          'autocomplete': 'off'
        }).addClass(this.settings.inputclass + ' form-control pincode-input-text').appendTo(wrapper); // calculate letter-spacing in Javascript since this isn't possible in CSS
        ...
        var touchtable = $('<div>').addClass('touch-flex').appendTo(wrapper); // create touch background elements (for showing user how many digits must be entered)
        ...
        var width = 100 / this.settings.inputs + '%';

        for (var _i = 0; _i < this.settings.inputs; _i++) {
          var _input = $('<input>').attr({
            'type': 'text',
            'maxlength': '1',
            'autocomplete': 'off',
            'placeholder': placeholders[_i] ? placeholders[_i] : undefined
          ...
          if (this.settings.hidedigits) {
            // hide digits
            _input.addClass('pincode-input-text-masked');
          } else {
            // show digits, also include default value
            _input.val(currentValue[_i]);
          }

          if (_i == 0) {
            _input.addClass('first');
          } else if (_i == this.settings.inputs - 1) {
            _input.addClass('last');
          } else {
            _input.addClass('mid');
          } // add events


          this._addEventsToInput(_input, _i + 1);
        }
      ...
      this._container.insertBefore(this.element); // error box
    ...
    _setValue: function _setValue(newValue, e) {
      // slice value
      ...
      if (this._isTouchDevice()) {
        ...
        $(this.element).select();
        $(this.element).blur();
        $(this.element).removeClass('noletterspacing'); // update original input box

        this.updateOriginalInput(); // oncomplete check
    ...
    _addEventsToInput: function _addEventsToInput(input, inputnumber) {
      ...
      input.on('keydown', $.proxy(function (e) {
        ...
        if (this._isTouchDevice()) {
          if (e.keyCode == 8 || e.keyCode == 46) {// do nothing on backspace and delete
...
//# sourceMappingURL=bootstrap-pincode-input.js.map